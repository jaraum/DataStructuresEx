...
bool IsLeaf(const HFMTNode *node) {
...
int CreateHuffmanCode(const BinaryTree *tree, HFMCodeTable *table) {
      if (!tree || !tree->root || !table) return ERROR;
      for (int i = 0; i < HFM_SYMBOL_RANGE; ++i) table->codes[i] = NULL;
